		...
		switch( $action ) {
			...
			case 'nuke':
				...
				if( !isset( $_POST['woo_st_nuke'] ) ) {
					$url = esc_url( add_query_arg( array( 'action' => null, 'message' => __( 'A required $_POST element was not detected so the requested nuke will not proceed', 'woo_ce' ) ) ) );
					wp_redirect( $url );
					exit();
				}
			...
			case 'autocomplete-order':
				$status = ( isset( $_POST['autocomplete_order'] ) ? $_POST['autocomplete_order'] : 0 );
