...
function imgmap_create_area_element($id, $title) {	
	...
	$coords = esc_attr(get_post_meta($id, 'coords', true));
	$titleArea = '';
	if (isset($meta->title_attribute) && !empty($meta->title_attribute)) {
		$coordsOptions = explode(',', $coords);
		$titleArea = '<div style="position: absolute; left: ' . ( $coordsOptions[0]) . 'px; top: '.( $coordsOptions[1]).'px; ">' . $meta->title_attribute . '</div>';
	}

	return $titleArea . '<area data-type="'.esc_attr($meta->type).'" data-tooltip="'.esc_attr($meta->type == 'tooltip' ? $meta->tooltip_text : false ). '" data-fill-color="'.esc_attr(str_replace('#', '', $color['fillColor'])).'" data-fill-opacity="'.esc_attr($color['fillOpacity']).'" data-stroke-color="'.esc_attr(str_replace('#', '', $color['strokeColor'])).'" data-stroke-opacity="'.esc_attr($color['strokeOpacity']).'" data-stroke-width="'.esc_attr($color['strokeWidth']).'" data-mapkey="area-'.$id.'" shape="poly" coords="' . $coords . '" href="'.esc_attr($link) .'" title="'.(isset($meta->title_attribute) ? $meta->title_attribute : $title).'" />';
}
